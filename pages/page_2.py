...
st.title('データ入力（単勝オッズ込み）')
st.caption('レース直前に指定した出馬表データを読み込んでテスト')

with st.form(key='profile_form'):
    
    # # セレクトボックス
    # race_id_list = st.selectbox(
    #     'レースID',
    ...
    if submit_btn:
        ...
        # テストデータの読み込み
        url = 'https://race.netkeiba.com/race/shutuba.html?race_id=' + race_id
        payload = {'url':'https://race.netkeiba.com/race/shutuba.html?race_id='+url,'renderType':'HTML','outputAsJson':'true'}
        payload = json.dumps(payload)
        payload = urllib.parse.quote(payload, safe = '')

        phantomjs_key = 'ak-j5nst-vv2fj-hg0my-qzkwr-kxa8q'
        phantom_url = 'https://phantomjscloud.com/api/browser/v2/' + phantomjs_key + '/?request=' + payload

        pj_res = requests.get(phantom_url)
        pj_res = pj_res.json()
        html = pj_res["content"]["data"]

        soup = BeautifulSoup(html, 'html.parser')
        odds_list = []
        odds = soup.find_all("td", attrs={'class': 'Txt_R Popular'})
        for odd in odds:
        ...
        stb.data_c['単勝'] = odds_list

        st.text('読み込み完了！')

        st.dataframe(stb.data_c)
...
# gain = gain(me.tansho_return_proper, X_test)